...
def main
  ...
  ARGV.each_with_index do | arg, i | read_htm_file(arg) end
end

def read_htm_file(url)
  # https://blog.takuros.net/entry/2014/04/15/070434

  return unless url.match( /.htm/ )
  return nil unless File.exist?( url )

  doc = Nokogiri::HTML( File.read( url ) )

  #st = []
  #[ '//p//b', '//font', '//td' ].each do | ii |
  #  xx = doc.xpath( ii ).size
  #  puts "#{url} #{ii} #{xx}"
  #end

  st = doc.xpath( '//p/b' ).size

  if st < 1
    # unless  myGitted.include( "op" )
    #   myGit( 'op', "!*.tex" )
    #   myGitted.push( "op" )
    # end
    def_booklet(  url )
...
def def_booklet( url )
  ...
  ff = "op/" + File.basename( url, ".htm" ) + ".tex"
  File.open( ff, "w" ) do | f |
...
def def_composer( url )
  ...
  doc
    ...
    composer = File.basename( url, ".htm" )
    ...
    doc.to_html
      ...
      # DONE 3-3) 小目次情報
      proc_033_composer_B_file( dir_p_a, composer_B_file )
...
def proc_033_composer_B_file( items, composer_B_filename )
  ...
  items.split(/<br>[[:space:]]*/).each do | item |
    ...
    if idx[0] == '■' # プログラム的には <p><b> を一レコードとして扱う
      ...
      if ( tmpORIG == "Op." ) && ( tmpTRAN == "作品" )
        composer_B_file.push( "\\input{op/#{idx[1]}.htm}" )
      else
...
def read_html_from_url_to_file( url )
  now = DateTime.now
  puts "#{now} #{url}"
  ...
  othtml = File.read( orig )
             .gsub( /朊/, '服' ) #「朊」は日本語では通常不使用文字、deepl 訳より「服」
             .gsub( /　Ｉ/, ' I' )
             .gsub( /(Mignon|Lieder)　+(I)/, '\1 \2')
             .gsub( /　(\d+)　/, ' \1 ')
             .gsub( /(Harfenspieler)(III)/, '\1 \2' )
             .gsub( /[[:blank:]]+$/ , '' )
             .gsub( /[[:blank:]]+(<br>)/ , '\1' )
             .gsub( /[[:blank:]]*Ⅰ/ , ' I ' )
             .gsub( /[[:blank:]]*Ⅱ/ , ' II ' )
             ...
             .gsub( /[[:blank:]]*第３集/, ' III ' )
             .gsub( /[[:blank:]]*第４集/, ' IV ' )
             .gsub( /([\p{hani}\p{hira}\p{kana}一])(I+)/ , '\1 \2 ' )
             .gsub( /[[:blank:]]*(I|II|III|IV)[[:blank:]]*(<)/ , ' \1 \2' )
             .gsub( /[[:blank:]]*(I|II|III|IV)[[:blank:]]+/ , ' \1 ' )
             .gsub( /(>)([^<>]+[^[:blank:]][^<>]+)[[:blank:]]+(<)/, '\1\2\3') # 最後
